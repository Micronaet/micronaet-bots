...
def cleanoutputdir():
    ''' Delete directory standard-out 'bots/botssys/out' 
        (as indicated in bos.ini). 
    ''' 
...
def comparerunresults(result_expect):
    ''' Result_expect is a dict that contains the expected results of a run.
        ...
        CompareRunResults(
            {
            'status': 0, 'lastreceived': 6, 'lasterror': 0, 'lastdone': 6, 
            'lastok': 0,'lastopen':0,'send': 4, 'processerrors': 0,
            'filesize': 6638})
    '''
...
def posttest(routestorun):
    ...
    # Run run first, save results in 'botssys/outfile' in 'botssys/infile' 
    # (so there is a directory 'botssys/infile/outfile'....)
